...
object TripRoutes:
  ...
  def listTripsEndpoint(xa: HikariTransactor[IO]): ServerEndpoint[Any, IO] =
    endpoint
      ...
      .tag("Trips")
      .serverLogic { (authHeader: String) => // Jawny typ wejściowy
  ...
  def createTripEndpoint(xa: HikariTransactor[IO]): ServerEndpoint[Any, IO] =
    endpoint
      ...
      .errorOut(statusCode.and(jsonBody[ErrorResponse]))
      .tag("Trips")
      .serverLogic { case (authHeader: String, req: CreateTripRequest) => // Jawne typowanie w pattern matchingu
        extractUserId(authHeader).flatMap {
          case Right(userId) =>
  ...
  def getTripDetailsEndpoint(xa: HikariTransactor[IO]): ServerEndpoint[Any, IO] =
    endpoint
      ...
      .errorOut(statusCode.and(jsonBody[ErrorResponse]))
      .tag("Trips")
      .serverLogic { case (tripId: UUID, authHeader: String) => // Jawne typowanie
        extractUserId(authHeader).flatMap {
          case Right(_) =>
  ...
  def updateTripEndpoint(xa: HikariTransactor[IO]): ServerEndpoint[Any, IO] =
    endpoint
      ...
      .errorOut(statusCode.and(jsonBody[ErrorResponse]))
      .tag("Trips")
      // Poprawione z case ((tripId, authHeader), req) na płaską krotkę z jawnymi typami
